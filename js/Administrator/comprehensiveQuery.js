...
function connect() {
	...
	stompClient.connect({}, function(frame) { //3连接webSocket的服务端。
		stompClient.subscribe('/user/manager/msg', function(respnose) {
			if(JSON.parse(respnose.body).code == "END") {
				waybillQueryListPost();
			}
...
function waybillQueryListHtml(list) {
	...
	for(var i = 0; i < list.length; i++) {
		...
		html += '<tr >' +
			...
			'<td>' + startTime + '</td>' +
			'<td >' + endTime + '</td>' +
			'<td width="6%" class="table-see">' +
			'<span onclick="getLocationHref(this)"  data-channelId="' + judgeAll(list[i].channelId) + '" data-orderId="' + judgeAll(list[i].orderId) + '">查看</span>' +
			'</td>' +
			'</tr>'
	}
	$("#tbodyBox").html(html);
